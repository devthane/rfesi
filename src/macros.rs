...
macro_rules! api_stream {
    (
        ...
        $(#[$m])*
        pub async fn $fn_name(&'a self, $( $param: $param_t, )*) -> EsiResult<impl Stream<Item = EsiResult<$ret_type>> + 'a> {
            let path = self
                .esi
                .get_endpoint_for_op_id($op_id)?
                $(
            ...
            Ok(self.esi.
                stream_query("GET", $visibility, path, None, None)
                .await)
        }
    };
    (
        ...
        pub async fn $fn_name(
            ...
            $($( $qparam: $qparam_t, )*)?
            $($( $opt_qparam: Option<$opt_qparam_t>, )*)?
        ) -> EsiResult<impl Stream<Item = EsiResult<$ret_type>> + 'a> {
            let path = self
                .esi
                .get_endpoint_for_op_id($op_id)?
                $(
            ...
            let params: Vec<(&str, &str)> = params.iter().map(|(a, b)| (*a, &**b)).collect();
            Ok(self.esi.
                stream_query("GET", $visibility, &path, Some(&params), None)
                .await)
        }
    };
}
